import { getAllFunkos, getFunkoById, createFunko, updateFunko, deleteFunko } from '../models/funkoPopModel.js';
...
export const getFunkos = async (req, res) => {
  try {
    ...
    if (req.query.categoriaId) {
      ...
      funkos = funkos.filter(f => f.categoria?._id.toString() === categoriaId);
    }
    // filtro por tipo
    if (req.query.tipo) {
...
export const addFunko = async (req, res) => {
  try {
    const { nombre, descripcion, precio, stock, imagen, tipo, categoriaId } = req.body;
    ...
    res.status(201).json({
      ...
      categoria: funkoConPopulated.categoria?.nombre || 'Categoría no encontrada',
...
export const updateFunkoData = async (req, res) => {
  try {
    ...
    if (!updated) {
      return res.status(404).json({ error: 'Funko no encontrado para actualizar' });
    }
    ...
    res.json({
      ...
      categoria: funkoActualizado.categoria?.nombre || 'Categoría no encontrada',
...
export const deleteFunkoData = async (req, res) => {
  try {
    ...
    if (!deleted) {
      return res.status(404).json({ error: 'Funko no encontrado para eliminar' });
    }
